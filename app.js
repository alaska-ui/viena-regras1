...
/* =========================================================
   MENU LATERAL — CATEGORIAS + SUBCATEGORIAS
========================================================= */
...
function renderNav(){
  ...
  nav.innerHTML = state.data.categories.map((category,index) => {
    ...
    const short = category.short || category.name || `Categoria ${index+1}`;
    const subcategories = Array.isArray(category.subcategories)
      ? category.subcategories
      : [];

    return `
      <div class="nav-category-group" data-category-group="${escapeAttr(id)}">
        <div class="nav-category-row">
          <button
            data-route="${escapeAttr(id)}"
            type="button"
            class="nav-category-main"
          >
          ...
          ${subcategories.length ? `
            <button
              ...
              data-toggle-category="${escapeAttr(id)}"
              aria-expanded="false"
        ...
        ${subcategories.length ? `
          <div class="nav-subcategories" data-subnav="${escapeAttr(id)}">
            ${subcategories.map((sub,subIndex) => {
              ...
              return `
                <button
                  ...
                  data-sub-route="${escapeAttr(id)}::${escapeAttr(subId)}"
                >
                  <span class="nav-sub-icon">${escapeHtml(icon)}</span>
                  <span class="nav-sub-name">${escapeHtml(subName)}</span>
                </button>
              `;
            }).join("")}
          </div>
  ...
  nav.querySelectorAll("[data-sub-route]").forEach(button => {
    button.addEventListener("click",event => {
      event.preventDefault();
      event.stopPropagation();
      const [categoryIdValue,subId] = button.dataset.subRoute.split("::");
      route(categoryIdValue);
      history.replaceState(null,"",`#${categoryIdValue}`);
      setTimeout(() => {
        document.getElementById(`subcat-${safeDomId(subId)}`)?.scrollIntoView({
          behavior:"smooth",
          block:"start"
        });
      },80);
...
/* =========================================================
   TODAS AS REGRAS — COMPATÍVEL COM REGRAS ANTIGAS E SUBCATEGORIAS
========================================================= */

function allRules(){
  return state.data.categories.flatMap((category,categoryIndex) => {
    const directRules = Array.isArray(category.rules) ? category.rules : [];
    const subs = Array.isArray(category.subcategories) ? category.subcategories : [];

    const direct = directRules.map((rule,ruleIndex) => ({
      cat:category,
    ...
    const nested = subs.flatMap((sub,subIndex) => {
...
/* =========================================================
   PÁGINA DA CATEGORIA — REGRAS + SUBCATEGORIAS
========================================================= */
...
function renderCategory(id){
  ...
  if($("catCode")) $("catCode").textContent = category.code ? `CÓDIGO ${category.code}` : "";
  if($("catTitle")) $("catTitle").textContent = category.name || category.short || `Categoria ${categoryIndex+1}`;
  if($("catDesc")) $("catDesc").textContent = category.desc || "";

  const categoryImage = assetUrl(category.image);
  const categoryHero = $("categoryHero");
  if(categoryHero){
    if(categoryImage){
      categoryHero.style.setProperty("--category-image",`url("${categoryImage}")`);
  ...
  const renderRule = rule => {
    ...
    const domIndex = globalRuleIndex++;
  ...
  if(ruleList){
    ...
    if(directRules.length){
      html += directRules.map(renderRule).join("");
    }

    subcategories.forEach((sub,subIndex) => {
      const subId = sub.id || `sub-${subIndex+1}`;
      const subName = sub.name || sub.title || `Subcategoria ${subIndex+1}`;
      const subDesc = sub.desc || sub.description || "";
      const subIcon = sub.icon || "◈";
      const rules = Array.isArray(sub.rules) ? sub.rules : [];

      html += `
        <section class="subcategory-section" id="subcat-${safeDomId(subId)}">
          <div class="subcategory-heading">
            <span class="subcategory-icon">${escapeHtml(subIcon)}</span>
            <div>
              <small>SUBCATEGORIA ${String(subIndex+1).padStart(2,"0")}</small>
              <h2>${escapeHtml(subName)}</h2>
              ${subDesc ? `<p>${escapeHtml(subDesc)}</p>` : ""}
            </div>
          </div>
          ${rules.length ? rules.map(renderRule).join("") : `<div class="empty-rules"><strong>NENHUMA REGRA CADASTRADA</strong><p>Esta subcategoria ainda não possui regras publicadas.</p></div>`}
        </section>
      `;
    });

    if(!html){
      html = `<div class="empty-rules"><strong>NENHUMA REGRA CADASTRADA</strong><p>Esta categoria ainda não possui regras publicadas.</p></div>`;
    }
  ...
  if(ruleToc){
    const subToc = subcategories.map((sub,subIndex) => {
      const subId = sub.id || `sub-${subIndex+1}`;
      const subName = sub.name || sub.title || `Subcategoria ${subIndex+1}`;
      return `<button type="button" data-scroll-sub="${escapeAttr(safeDomId(subId))}">${escapeHtml(sub.icon || "◈")} ${escapeHtml(subName)}</button>`;
    }).join("");

    const ruleTocHtml = tocItems.map(item => `
      <button type="button" data-scroll-rule="${item.domIndex}">
        ${escapeHtml(item.code)}${item.code && item.title ? " — " : ""}${escapeHtml(item.title)}
      </button>
    `).join("");

    ruleToc.innerHTML = (subToc || ruleTocHtml)
      ? `<strong>NESTA CATEGORIA</strong>${subToc}${ruleTocHtml}`
      : "";

    ruleToc.querySelectorAll("[data-scroll-sub]").forEach(button => {
      button.addEventListener("click",() => {
        document.getElementById(`subcat-${button.dataset.scrollSub}`)?.scrollIntoView({behavior:"smooth",block:"start"});
      });
    });

    ruleToc.querySelectorAll("[data-scroll-rule]").forEach(button => {
      button.addEventListener("click",() => {
        document.getElementById(`rule-${button.dataset.scrollRule}`)?.scrollIntoView({behavior:"smooth",block:"center"});
      });
    });
  }
